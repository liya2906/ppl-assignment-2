...
export const class2proc = (exp: ClassExp): ProcExp =>
    makeProcExp(
        ...
        [
            makeProcExp(
                [makeVarDecl("msg")],
                [makeMethodDispatch(exp.methods)]
            )
        ]
    );
...
const transformCExp = (exp: CExp): Result<CExp> =>
    ...
    isLetExp(exp) ?
        bind(
            mapResult(
                (b: Binding) =>
                    mapv(transformCExp(b.val), (val: CExp) =>
                        makeBinding(b.var.var, val)),
                exp.bindings
            ),
            (bindings: Binding[]) =>
                mapv(mapResult(transformCExp, exp.body), (body: CExp[]) =>
...
const transformExp = (exp: Exp): Result<Exp> =>
    isDefineExp(exp) ?
        mapv(transformCExp(exp.val), (val: CExp) =>
            makeDefineExp(exp.var, val)) :
        transformCExp(exp);

export const transform = (exp: Exp | Program): Result<Exp | Program> =>
    isProgram(exp) ?
        mapv(mapResult(transformExp, exp.exps), (exps: Exp[]) =>
            makeProgram(exps)) :
        transformExp(exp);