...
const parseClassExp = (fields: Sexp, methods: Sexp): Result<ClassExp> => {
    ...
    if (!isGoodBindings(methods)) {
        return makeFailure(`Malformed class methods`);
    }
    ...
    const bindingsResult = mapv(valsResult,
        (vals: CExp[]) => zipWith(makeBinding, methodNames, vals));
    return mapv(bindingsResult,
        (bindings: Binding[]) => makeClassExp(vars, bindings));
};
